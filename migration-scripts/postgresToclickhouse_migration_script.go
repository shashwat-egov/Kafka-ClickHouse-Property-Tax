...
func migratePropertyAddress(ctx context.Context, pgPool *pgxpool.Pool, chConn clickhouse.Conn, batchSize, workers int, globalCounter *int64, resume bool, tenant string) (int64, error) {
	...
	const pgQuery = `
		...
		INNER JOIN eg_pt_address a ON p.id = a.propertyid
		WHERE p.tenantid = $1`
...
func migratePaymentWithDetails(
    ...
    return parallelByTenant(
        ...
        func(scan func(dest ...any) error, appendFn func(v ...any) error) error {
            ...
            // ✅ NEW: compute financial year from transaction_date
            fy := computeFinancialYear(txnDateMs)

            // ✅ OPTIONAL fallback (recommended)
            if fy == "" {
                fy = computeFinancialYear(createdTimeMs)
            }
...
func migrateBill(
    ...
    if resume {
        ...
        skipTenants, err = getCompletedTenants(
            ...
            `SELECT tenant_id, count() FROM bill_entity_v1 FINAL GROUP BY tenant_id`,
        )
        if err != nil {
            return 0, err
        }
    ...
    return parallelByTenant(
        ...
        func(scan func(dest ...any) error, appendFn func(v ...any) error) error {
			...
			// ✅ Compute financial year from created_time
    		fy := computeFinancialYear(createdTimeMs)
...
func migratePropertyUnit(ctx context.Context, pgPool *pgxpool.Pool, chConn clickhouse.Conn, batchSize, workers int, globalCounter *int64, resume bool, tenant string) (int64, error) {
	...
	const chInsert = `INSERT INTO property_unit_entity_v1 (
	...
	return parallelByTenant(ctx, pgPool, chConn,
		"property_unit_entity_v1", tenantQuery, pgQuery, chInsert,
...
func createDemandStagingTables(ctx context.Context, chConn clickhouse.Conn) error {
	queries := []string{
		`CREATE TABLE IF NOT EXISTS _stg_demand_v1 (
		...
		`CREATE TABLE IF NOT EXISTS _stg_demanddetail_v1 (
...
func dropDemandStagingTables(ctx context.Context, chConn clickhouse.Conn) {
	_ = chConn.Exec(ctx, "DROP TABLE IF EXISTS _stg_demand_v1")
	_ = chConn.Exec(ctx, "DROP TABLE IF EXISTS _stg_demanddetail_v1")
}
...
func migrateDemandRaw(ctx context.Context, pgPool *pgxpool.Pool, chConn clickhouse.Conn, batchSize, workers int, globalCounter *int64, skipTenants map[string]bool, tenantFilter string) (int64, error) {
	...
	const chInsert = `INSERT INTO _stg_demand_v1`

	return parallelByTenant(ctx, pgPool, chConn,
		"_stg_demand_v1", tenantQuery, pgQuery, chInsert,
...
func migrateDemandDetailRaw(ctx context.Context, pgPool *pgxpool.Pool, chConn clickhouse.Conn, batchSize, workers int, globalCounter *int64, skipTenants map[string]bool, tenantFilter string) (int64, error) {
	...
	const chInsert = `INSERT INTO _stg_demanddetail_v1`

	return parallelByTenant(ctx, pgPool, chConn,
		"_stg_demanddetail_v1", tenantQuery, pgQuery, chInsert,
...
INSERT INTO demand_with_details_entity (
    ...
    pt_time_interest,
    pt_tax_collection,
    pt_cancer_cess_collection,
    pt_fire_cess_collection,
    pt_roundoff_collection,
    pt_owner_exemption_collection,
    pt_unit_usage_exemption_collection,
    pt_advance_carryforward_collection,
    pt_decimal_ceiling_debit_collection,
    pt_time_rebate_collection,
    pt_decimal_ceiling_credit_collection,
    pt_time_penalty_collection,
    pt_adhoc_penalty_collection,
    pt_adhoc_rebate_collection,
    pt_time_interest_collection,
    outstanding_amount,
    is_paid,
    created_by,
    created_time,
...
SELECT
	...
	d.billexpirytime, 
	d.fixedbillexpirydate,

	sum(ifNull(dd.taxamount, 0)),
	sum(ifNull(dd.collectionamount, 0)),

	sumIf(ifNull(dd.taxamount, 0), dd.taxheadcode = 'PT_TAX'),
	sumIf(ifNull(dd.taxamount, 0), dd.taxheadcode = 'PT_CANCER_CESS'),
	sumIf(ifNull(dd.taxamount, 0), dd.taxheadcode = 'PT_FIRE_CESS'),
	sumIf(ifNull(dd.taxamount, 0), dd.taxheadcode = 'PT_ROUNDOFF'),
	sumIf(ifNull(dd.taxamount, 0), dd.taxheadcode = 'PT_OWNER_EXEMPTION'),
	sumIf(ifNull(dd.taxamount, 0), dd.taxheadcode = 'PT_UNIT_USAGE_EXEMPTION'),
	sumIf(ifNull(dd.taxamount, 0), dd.taxheadcode = 'PT_ADVANCE_CARRYFORWARD'),
	sumIf(ifNull(dd.taxamount, 0), dd.taxheadcode = 'PT_DECIMAL_CEILING_DEBIT'),
	sumIf(ifNull(dd.taxamount, 0), dd.taxheadcode = 'PT_TIME_REBATE'),
	sumIf(ifNull(dd.taxamount, 0), dd.taxheadcode = 'PT_DECIMAL_CEILING_CREDIT'),
	sumIf(ifNull(dd.taxamount, 0), dd.taxheadcode = 'PT_TIME_PENALTY'),
	sumIf(ifNull(dd.taxamount, 0), dd.taxheadcode = 'PT_ADHOC_PENALTY'),
	sumIf(ifNull(dd.taxamount, 0), dd.taxheadcode = 'PT_ADHOC_REBATE'),
	sumIf(ifNull(dd.taxamount, 0), dd.taxheadcode = 'PT_TIME_INTEREST'),

	sumIf(ifNull(dd.collectionamount, 0), dd.taxheadcode = 'PT_TAX'),
	sumIf(ifNull(dd.collectionamount, 0), dd.taxheadcode = 'PT_CANCER_CESS'),
	sumIf(ifNull(dd.collectionamount, 0), dd.taxheadcode = 'PT_FIRE_CESS'),
	sumIf(ifNull(dd.collectionamount, 0), dd.taxheadcode = 'PT_ROUNDOFF'),
	sumIf(ifNull(dd.collectionamount, 0), dd.taxheadcode = 'PT_OWNER_EXEMPTION'),
	sumIf(ifNull(dd.collectionamount, 0), dd.taxheadcode = 'PT_UNIT_USAGE_EXEMPTION'),
	sumIf(ifNull(dd.collectionamount, 0), dd.taxheadcode = 'PT_ADVANCE_CARRYFORWARD'),
	sumIf(ifNull(dd.collectionamount, 0), dd.taxheadcode = 'PT_DECIMAL_CEILING_DEBIT'),
	sumIf(ifNull(dd.collectionamount, 0), dd.taxheadcode = 'PT_TIME_REBATE'),
	sumIf(ifNull(dd.collectionamount, 0), dd.taxheadcode = 'PT_DECIMAL_CEILING_CREDIT'),
	sumIf(ifNull(dd.collectionamount, 0), dd.taxheadcode = 'PT_TIME_PENALTY'),
	sumIf(ifNull(dd.collectionamount, 0), dd.taxheadcode = 'PT_ADHOC_PENALTY'),
	sumIf(ifNull(dd.collectionamount, 0), dd.taxheadcode = 'PT_ADHOC_REBATE'),
	sumIf(ifNull(dd.collectionamount, 0), dd.taxheadcode = 'PT_TIME_INTEREST'),

	sum(ifNull(dd.taxamount, 0)) - sum(ifNull(dd.collectionamount, 0)),

	toUInt8(
		if(
			sum(ifNull(dd.taxamount, 0)) - sum(ifNull(dd.collectionamount, 0)) <= 0,
			1,
			0
		)
	),
	...
	d.createdby, 
	fromUnixTimestamp64Milli(d.createdtime, 'Asia/Kolkata'),
	d.lastmodifiedby, 
	fromUnixTimestamp64Milli(d.lastmodifiedtime, 'Asia/Kolkata')

FROM _stg_demand_v1 d
LEFT JOIN _stg_demanddetail_v1 dd 
  ON dd.tenantid = d.tenantid 
 AND dd.demandid = d.id

GROUP BY
	d.tenantid, d.id, d.consumercode, d.consumertype, d.businessservice, d.payer,
	d.taxperiodfrom, d.taxperiodto, d.status,
...
func migrateDemandWithDetails(ctx context.Context, pgPool *pgxpool.Pool, chConn clickhouse.Conn, batchSize, workers int, globalCounter *int64, resume bool, tenant string) (int64, error) {
	...
	log.Println("[demand] Phase 1: Migrating demand data into staging...")
	demandCount, err := migrateDemandRaw(ctx, pgPool, chConn, batchSize, workers, globalCounter, skipTenants, tenant)
	if err != nil {
		return 0, fmt.Errorf("migrate demand: %w", err)
	}

	log.Printf("[demand] Phase 1 complete: %d demand rows staged", demandCount)
...
//         SELECT
//             COALESCE(bd.id,''),
//             COALESCE(bd.tenantid,''),
//             COALESCE(bd.demandid,''),
//             COALESCE(bd.billid,''),
//             COALESCE(bd.amount,0)::float8,
//             COALESCE(bd.amountpaid,0)::float8,
//             COALESCE(bd.fromperiod,0),
//             COALESCE(bd.toperiod,0),
//             COALESCE(bd.additionaldetails::text,'{}'),
//             COALESCE(bd.channel,''),
//             COALESCE(bd.voucherheader,''),
//             COALESCE(bd.boundary,''),
//             COALESCE(bd.collectiontype,''),
//             COALESCE(bd.billdescription,''),
//             COALESCE(bd.expirydate,''),
//             COALESCE(bd.displaymessage,''),
//             COALESCE(bd.callbackforapportioning,''),
//             COALESCE(bd.cancellationremarks,''),

//             -- join with bill for audit fields
//             COALESCE(b.createdby,''),
//             COALESCE(b.createdtime,0),
//             COALESCE(b.lastmodifiedby,''),
//             COALESCE(b.lastmodifiedtime,0)

//         FROM egcl_billdetial bd
//         LEFT JOIN egcl_bill b ON bd.billid = b.id
//         WHERE bd.tenantid = $1`

//     const chInsert = `
//         INSERT INTO bill_detail_entity_v1 (
//             id,
//             tenant_id,
//             demand_id,
//             bill_id,
...
//         skipTenants, err = getCompletedTenants(
//             ctx, pgPool, chConn,
//             "bill_detail_entity_v1",
//             `SELECT tenantid, count(*) FROM egcl_billdetial GROUP BY tenantid`,
//             `SELECT tenant_id, count() FROM bill_detail_entity_v1 FINAL GROUP BY tenant_id`,
//         )
//         if err != nil {
//             return 0, err
//         }
...
//             var (
//                 id, tenantID, demandID, billID string
//                 amount, amountPaid float64
//                 fromPeriod, toPeriod int64
//                 additionalDetails string
//                 channel, voucherHeader, boundary string
//                 collectionType, billDescription string
//                 expiryDate, displayMessage string
//                 callbackForApportioning string
...
//             if err := scan(
//                 &id, &tenantID, &demandID, &billID,
//                 &amount, &amountPaid,
//                 &fromPeriod, &toPeriod,
//                 &additionalDetails,
//                 &channel, &voucherHeader, &boundary,
//                 &collectionType, &billDescription,
//                 &expiryDate, &displayMessage,
...
//             // ✅ Financial Year from from_period
//             fy := computeFinancialYear(fromPeriod)
//             if fy == "" {
//                 fy = computeFinancialYear(toPeriod)
//             }
...
func migrateBillDetail(
    ...
    if resume {
        ...
        skipTenants, err = getCompletedTenants(
            ctx, pgPool, chConn,
            "bill_detail_entity_v1",
            `SELECT DISTINCT tenantid FROM egcl_billdetial`,
            `SELECT DISTINCT tenant_id FROM bill_detail_entity_v1`,
        )
        if err != nil {
            return 0, err
        }
    ...
    return parallelByTenant(
        ...
        func(scan func(dest ...any) error, appendFn func(v ...any) error) error {
            ...
            // ✅ Trim strings (safe cleanup)
            tenantID = strings.TrimSpace(tenantID)
            billID = strings.TrimSpace(billID)

            // ✅ JSON validation (VERY IMPORTANT)
            if !json.Valid([]byte(additionalDetails)) {
                additionalDetails = "{}"
            }

            // ✅ Financial Year logic (safe fallback)
            fy := computeFinancialYear(fromPeriod)
            if fy == "" {
                fy = computeFinancialYear(toPeriod)
            }
            ...
            // ✅ Retry logic for ClickHouse insert
            var err error
            for i := 0; i < 3; i++ {
                err = appendFn(
                    id,
...
// ─── Table: property_audit_entity (from eg_pt_property_audit) ─────────
...
func migratePropertyAudit(
	...
	const chInsert = `
		INSERT INTO property_audit_entity (
...
var allTables = []string{"property_address", "property_unit", "property_owner", "demand_details", "assessment", "payment_with_details", "bill", "bill_detail", "property_audit"}
...
func main() {
	...
	flag.StringVar(&pgDB, "pg-db", "testdb", "PostgreSQL database (required)")
	flag.StringVar(&pgUser, "pg-user", "postgres", "PostgreSQL user")
	flag.StringVar(&pgPassword, "pg-password", "postgres", "PostgreSQL password")
